...
Features:
1. Strict Validation:
   - Prohibits classDiagram, stateDiagram, erDiagram, gitGraph with Thai/Non-ASCII (lexer crash prevention)
...
3. Markdown Document Scanner:
   - Extracts all ```mermaid blocks from reports/markdown
...
class MermaidUnicodeGuardian:
    ...
    def extract_diagram_type(cls, mermaid_code: str) -> Tuple[str, str]:
        """
        Extracts diagram type (e.g. 'flowchart', 'graph', 'classDiagram').
    ...
    def validate(cls, mermaid_code: str) -> Dict[str, Any]:
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            # 3. Check for raw unquoted brackets containing Thai/Non-ASCII
            unquoted_square = re.finditer(r'\b([a-zA-Z0-9_]+)\[([^"\]\n]*[\u0E00-\u0E7F][^"\]\n]*)\]', line)
            for m in unquoted_square:
                node_id = m.group(1)
                errors.append(MermaidSyntaxErrorDetail(
                    rule="Mandatory Quotes: Unquoted Square Label",
                    line_number=idx,
            ...
            # 4. Check for Thai/Non-ASCII in Node IDs
            line_outside_quotes = re.sub(r'"[^"\n]*"', '""', line)
            thai_node_id_matches = re.finditer(
                r'([\u0E00-\u0E7F][\u0E00-\u0E7F0-9_]*)\s*(\[|\(|\{|\-\-|\-\.\-|==>|-->)', 
                line_outside_quotes
            )
    ...
    def sanitize(cls, mermaid_code: str) -> Tuple[str, List[str]]:
        ...
        4. Formats subgraphs properly with quoted labels
        """
        fixes: List[str] = []
        lines = mermaid_code.strip().splitlines()
        if not lines:
    ...
    def audit_markdown_text(cls, markdown_text: str) -> Dict[str, Any]:
        ...
        validates them, and provides fully sanitized markdown.
        """
